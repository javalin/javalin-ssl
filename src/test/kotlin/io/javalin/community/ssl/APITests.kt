...
import io.javalin.community.ssl.SSLPlugin.Companion.SSL
...
class APITests : IntegrationTestClass(){
    ...
    fun `complete api compiles`(){
        ...
        val certInputStream: java.io.InputStream = TODO()
        ...
        var plugin = SSLPlugin {
            ...
            it.http3=false;                                                          // Toggle HTTP/3 Support
            ...
            it.http3Port=443;                                                        // Port to use on the http3 connector (UDP)
            it.redirect=false;                                                       // Redirect all http requests to https
            it.disableHttp3Upgrade=false;                                            // Disable the HTTP/3 upgrade header
            ...
            it.pemFromInputStream(certInputStream,keyInputStream);                   // load from the given input streams
            it.pemFromInputStream(certInputStream,keyInputStream,"keyPassword");     // load from the given input streams with the given key password
        ...
        SSLPlugin{
            it.withTrustConfig{
                ...
                it.certificateFromInputStream(certInputStream);                 // load a PEM/DER/P7B cert from the given input stream
                ...
                it.trustStoreFromInputStream(certInputStream, "password");      // load a trust store from the given input stream
            }
        }
